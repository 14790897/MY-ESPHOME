...
系统同时集成 **SNTP 时间同步** 与 **MQTT 上行**。后端网站（https://iot2.14790897.xyz/）对 MQTT 上传进行 Token 鉴权，鉴权通过后将数据写入 PostgreSQL，前端按需读取展示。方案兼具**低时延、低功耗、免配网**与**离线可用（ESP‑NOW 本地通信）**的特点，适用于家庭与办公场景的本地环境质量看板。
